...
open class KeyboardLayoutGuide: UILayoutGuide {
    ...
    private func keyboardWillChangeFrame(_ note: Notification) {
        if let height = note.keyboardHeight, let duration = note.animationDuration {
            if #available(iOS 11.0, *), usesSafeArea, height > 0, let _ = owningView?.safeAreaInsets.bottom {
                //height != bottom
            }
